...
@dataclass
class GeCoAdapterConfig:
    type: str = field(default='geco_adapter', metadata={"help": "Type of Low-rank Adaptation."})
    ...
    rank_ramp: bool = field(default=False, metadata={"help": "Utilizing Rank ramp structure, progressively scailing up rank of GeCoAdapter."})
    lora_alpha: int = field(default=None, metadata={"help": "Lora alpha"})
    lora_dropout: float = field(default=0, metadata={"help": "Lora dropout"})
    lora_weight_init: str = field(default="geco_niter_4", 
        metadata={"help": "Initialization scheme for lora_A and lora_B, Initialize the GeCo adapter with fast SVD, which completes in just a few seconds."})
    ...
    def __post_init__(self):
        self.peft_type = "GeCoAdapter"


class GeCoAdapterModel(nn.Module):
            ...
            if target_module_found:
                ...
                if isinstance(module, nn.Linear):
                    ...
                    with gather_params_ctx(new_module.weight):
                        new_module.geco_init(self.peft_config.lora_weight_init)
...
class Linear(nn.Linear, LoraLayer):
    ...
    def geco_init(self, init):
        weight = self.weight
        dtype = weight.dtype
        if dtype not in [torch.float32, torch.float16, torch.bfloat16]:
            raise TypeError(
                "Please initialize GeCoAdapter under float32, float16, or bfloat16. "
                "Subsequently, re-quantize the residual model to help minimize quantization errors."
            )
        weight = weight.to(torch.float32)
        
        if init == "geco_adapter":
            # Perform SVD and initialize GeCoAdapter with singular components
            ...
            raise ValueError(
                f"init should be 'geco_adapter' or 'geco_niter_[number of iters]', got {init} instead."
            )
